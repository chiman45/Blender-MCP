...
class MCPClient:
    ...
    async def call_llm(self):
        try:
            ...
            for message in self.messages:
                if message["role"] == "user":
                    if isinstance(message["content"], str):
                        current_content = [{"text": message["content"]}]
                    else:
                        current_content = []
                        for content_item in message["content"]:
                            if isinstance(content_item, dict):
                                if content_item.get("type") == "tool_result":
                                    current_content.append({"text": f"Tool result: {content_item.get('content', '')}"})
                                else:
                                    current_content.append({"text": str(content_item)})
                            else:
                                current_content.append({"text": str(content_item)})
                    ...
                    if isinstance(message["content"], str):
                        ...
                        # Handle tool calls - for now, just add as text
            ...
            latest_message = chat_history[-1]["parts"][0]["text"] if chat_history else ""
            response = chat.send_message(latest_message)
            
            # Create a mock response object similar to Claude's format
            class MockResponse:
